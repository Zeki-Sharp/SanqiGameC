...
public class BlockPlacementManager : MonoBehaviour
{
    ...
    private void Update()
    {
        if (isPlacing)
        {
            ...
            UpdatePreviewTowerGroupFollowMouse();
        }
    }
    ...
    /// 更新预览塔组位置
    /// </summary>
    /// <param name="baseGridPos">基础网格位置</param>
            ...
            foreach (var collider in towers)
            {
                ...
                if (collider.TryGetComponent<Tower>(out var tower))
                {
                    ...
                    if (collider.transform.position == towerObject.transform.position)
                    {
                        ...
                        if (shouldUpdate)
                        {
                            ChangePreviewTowersColor(canReplaceColor, towerObject.gameObject);
                        }
                        else
                        {
                            ChangePreviewTowersColor(canPlaceColor, towerObject.gameObject);
                        }
                    }
                }
            }
