...
{
    [Header("测试配置")] [SerializeField] private GameMap gameMap;
    ...
    private void Start()
    {
        if (runTestsOnStart)
        {
            // 确保 BlockPlacementManager 已经加载了预制体
            var placementMgr = FindFirstObjectByType<BlockPlacementManager>();
            placementMgr?.SendMessage("LoadTowerPrefab");
    ...
    public void Test_01()
    {
        ...
        foreach (var vector2 in config.GetCellCoords())
        {
            ...
            if (tower == null)
            {
                Debug.LogWarning($"TowerData {tower?.name} 没有预制体，用测试塔代替");
                tower = CreateTestTowerData();
            }

            towerDatas.Add(tower);
        }
    ...
    private TowerData CreateTestTowerData()
    {
        ...
        var attackField = type.GetField("physicAttack",
            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        var rangeField = type.GetField("attackRange",
            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        var intervalField = type.GetField("attackInterval",
            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
